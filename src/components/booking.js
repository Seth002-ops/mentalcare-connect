...
const Booking = () => {
  ...
  const handleBook = async () => {
    if (selectedTherapist && selectedDate && selectedTime) {
      ...
      try {
        const scheduledTime = `${selectedDate}T${selectedTime.replace(' ', '').replace(':', ':00')}:00`;
        ...
        navigate('/payment', { state: { bookingId: data.booking_id } });
      } catch (error) {
            ...
            {selectedTherapist && (
              <>
                ...
                <button
                  ...
                  style={{
                    ...styles.btnPrimary,
                    backgroundColor: !selectedTherapist || !selectedDate || !selectedTime 
                      ? '#D1D5DB' : '#2BB3A3',
