...
const MoodFace = ({ type, color }) => {
  ...
  switch (type) {
    case 'bigsmile':
      mouth = <path d="M8 13.5 q4 4.5 8 0" stroke={color} />;
      break;
    case 'smile':
      mouth = <path d="M8.5 14 q3.5 3 7 0" stroke={color} />;
      break;
    case 'soft':
      mouth = <path d="M9 14.5 q3 2 6 0" stroke={color} />;
      break;
    case 'neutral':
      mouth = <line x1="9" y1="14.5" x2="15" y2="14.5" stroke={color} />;
      break;
    case 'frown':
      mouth = <path d="M8.5 15.5 q3.5 -3 7 0" stroke={color} />;
      break;
    case 'deep':
      mouth = <path d="M8 16 q4 -4 8 0" stroke={color} />;
      extra = (
        <>
          <line x1="7" y1="7.5" x2="10.5" y2="8.5" stroke={color} />
          <line x1="17" y1="7.5" x2="13.5" y2="8.5" stroke={color} />
        </>
      );
      break;
    case 'tear':
      mouth = <path d="M8.5 15.5 q3.5 -3 7 0" stroke={color} />;
      extra = <path d="M16.5 12 q1 1.6 0 2.4 q-1 -0.8 0 -2.4" fill={color} stroke="none" />;
      break;
    case 'open':
      mouth = <ellipse cx="12" cy="14.5" rx="2.5" ry="2" stroke={color} />;
      break;
    default:
      mouth = <path d="M8.5 14 q3.5 3 7 0" stroke={color} />;
  }
...
const MOODS = [
  ...
  { value: 'calm', label: 'Calm', type: 'calm', color: '#43A047', score: 7 },
...
const AvatarPreset = ({ v, size = 40 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" style={{ borderRadius: '50%', display: 'block' }}>
      ...
      {p.hair === 'short' && (
        <path d="M14.5 20 a9.5 9.5 0 0 1 19 0" fill="none" stroke={p.hairColor} strokeWidth="4.5" strokeLinecap="round" />
      )}
      {p.hair === 'long' && (
        <>
          <path d="M14.5 20 a9.5 9.5 0 0 1 19 0" fill="none" stroke={p.hairColor} strokeWidth="4.5" strokeLinecap="round" />
          <path d="M14.5 20 v8" stroke={p.hairColor} strokeWidth="4" strokeLinecap="round" />
          <path d="M33.5 20 v8" stroke={p.hairColor} strokeWidth="4" strokeLinecap="round" />
        </>
      )}
      {p.hair === 'bun' && (
        <>
          <path d="M14.5 20 a9.5 9.5 0 0 1 19 0" fill="none" stroke={p.hairColor} strokeWidth="4.5" strokeLinecap="round" />
          <circle cx="24" cy="9" r="4" fill={p.hairColor} />
        </>
      )}
      {p.hair === 'curly' && (
        <>
          <circle cx="17" cy="13" r="3.5" fill={p.hairColor} />
          <circle cx="24" cy="10.5" r="3.8" fill={p.hairColor} />
          <circle cx="31" cy="13" r="3.5" fill={p.hairColor} />
        </>
      )}
      {p.hair === 'none' && (
        <path d="M15 16 a9.5 9.5 0 0 1 18 0" fill="none" stroke={p.hairColor} strokeWidth="2" strokeLinecap="round" />
      )}
    </svg>
  );
};
...
const WELLNESS_TIPS = [
  ...
  { title: 'Body Scan', text: 'Close your eyes and slowly focus on each part of your body from head to toe.' },
];
    ...
    <header className="cd-header">
      <div className="cd-header-inner">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <Skeleton w="92px" h="18px" />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Skeleton w="40px" h="40px" r="12px" />
          <Skeleton w="118px" h="38px" r="999px" />
        </div>
      </div>
    </header>

    <main className="cd-main">
      <Skeleton h="132px" r="18px" style={{ marginBottom: '1.5rem' }} />

      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.75rem', flexWrap: 'wrap' }}>
        {[0, 1, 2].map((i) => (
          <React.Fragment key={i}>
            <Skeleton w="34px" h="34px" r="50%" />
            <Skeleton w="88px" h="14px" />
            {i < 2 && <Skeleton w="70px" h="2px" r="2px" style={{ flex: 1, minWidth: '24px' }} />}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="cd-grid">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="cd-card">
            <Skeleton w="210px" h="20px" style={{ marginBottom: '1rem' }} />
            <div className="cd-mood-grid">
              {Array.from({ length: 12 }).map((_, i) => (
                <Skeleton key={i} h="74px" r="12px" />
              ))}
            </div>
          </div>

          <div className="cd-card">
            <Skeleton w="180px" h="20px" style={{ marginBottom: '1rem' }} />
            <Skeleton h="150px" r="14px" />
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {['Next Session', 'Past Sessions', 'AI Companion', 'Daily Tip'].map((title, idx) => (
            <div key={title} className="cd-card">
              <Skeleton w={idx === 2 ? '130px' : '150px'} h="18px" style={{ marginBottom: '1rem' }} />
              <Skeleton h={idx === 2 ? '138px' : idx === 3 ? '120px' : '96px'} r="12px" />
            </div>
          ))}
        </div>
      </div>
...
// It is a neutral typographic placeholder until your real logo is ready.
const BrandMark = ({ size = 34 }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem' }}>
    ...
    <span style={{ fontWeight: 900, fontSize: '1rem', color: '#111827', letterSpacing: '-0.02em' }}>
      MECAC
    </span>
  </div>
);

const AvatarDisplay = ({ avatar, userName, size = 40 }) => {
...
const ClientDashboard = ({ logout }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const [profileRes, bookingsRes, moodTodayRes, weekRes] = await Promise.allSettled([
        fetch(`${API_URL}/users/me`, { headers }),
        fetch(`${API_URL}/bookings/me`, { headers }),
        fetch(`${API_URL}/moods/today`, { headers }),
        fetch(`${API_URL}/moods/week`, { headers }),
      ]);
      ...
      if (profileRes.status === 'fulfilled') {
        ...
        if (res.ok) {
          const data = await res.json();
          setUserName(data.name || data.email?.split('@')[0] || 'Client');
          setUserEmail(data.email || '');

          if (data.avatar) {
            setAvatar(data.avatar);
          } else if (data.profile_photo_url) {
            setAvatar({ type: 'upload', url: data.profile_photo_url });
          } else {
            setAvatar(null);
          }
        ...
        addToast('Network error loading profile.', 'error');
      }
      ...
      // Mood today
      if (moodTodayRes.status === 'fulfilled' && moodTodayRes.value.ok) {
        const data = await moodTodayRes.value.json();
        if (data && (data.mood || data.logged || data.exists)) {
  ...
  }, [addToast, defaultWeek, logout]);

  useEffect(() => {
    const dayIndex = new Date().getDate() % WELLNESS_TIPS.length;
    setDailyTip(WELLNESS_TIPS[dayIndex]);
  }, []);

  const handleMoodSelect = (m) => {
    if (alreadyLogged || moodSubmitted) return;
    setSelectedMood(m);
  ...
  const handleMoodSubmit = async () => {
    if (!selectedMood) {
      setMoodError('Please choose a mood first.');
      return;
    }
    ...
    const token = localStorage.getItem('token');
    if (!token) {
      setMoodError('Please log in again.');
      setMoodSaving(false);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setWeekData((prev) =>
          prev.map((day, idx) =>
            idx === prev.length - 1 ? { ...day, mood: { ...selectedMood } } : day
          )
        );
      } else {
        const data = await res.json().catch(() => ({}));
        setMoodError(data.detail || 'Could not save your mood.');
      }
    } catch (err) {
  ...
  const handleBookingSuccess = () => {
    addToast('Session request received. Check your bookings.', 'success');
    refreshBookings();
  };
  ...
  const handleUpload = async (e) => {
    ...
    setAvatar({ type: 'upload', url: previewUrl });

    const token = localStorage.getItem('token');
    if (!token) {
      addToast('Photo saved locally only. Please log in.', 'info');
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const finalUrl = data.url || data.profile_photo_url || previewUrl;
        setAvatar({ type: 'upload', url: finalUrl });
        addToast('Profile photo updated.', 'success');
      } else {
        addToast('Photo saved locally only. Server update failed.', 'info');
      }
    } catch (err) {
      addToast('Photo saved locally only. Network issue.', 'info');
    } finally {
      e.target.value = '';
    }
  };
  ...
  const saveAvatar = async (payload) => {
    ...
    try {
      ...
      addToast('Avatar saved locally.', 'info');
    }
  };
  ...
  const quickActions = [
    { title: 'Find a Therapist', desc: 'Browse licensed professionals', to: '/therapists', icon: <IconUsers /> },
    { title: 'Book a Session', desc: 'Schedule online therapy', to: '/booking', icon: <IconCalendar /> },
    { title: 'AI Companion', desc: 'Talk anytime, 24/7', to: '/chat/1', icon: <IconBot /> },
    { title: 'Messages', desc: 'Check secure chat', to: '/chat/1', icon: <IconMessage /> },
  ];

  const upcomingSession = useMemo(() => {
    return (
      bookings
        .filter((b) => ['confirmed', 'completed'].includes(b.status) && new Date(b.scheduled_time) >= new Date())
  ...
  const pts = weekData.map((d, i) => {
    ...
    const score = d.mood?.score ?? 5;
    const y = chartH - pad - ((score - 1) / 9) * (chartH - 2 * pad);
    return [x, y];
  });
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F9FAFB' }}>
      ...
      <header className="cd-header">
        <div className="cd-header-inner">
          <button className="cd-icon-btn" onClick={() => setDrawerOpen(true)} aria-label="Open menu">
            <IconMenu />
          </button>

          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
            <BrandMark size={34} />
            <div className="cd-avatar-text" style={{ minWidth: 0 }}>
              <div
                style={{
                  fontWeight: 700,
                  color: '#111827',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <NotificationBell />
            <button className="cd-avatar-chip" onClick={() => setAvatarModalOpen(true)}>
              <AvatarDisplay avatar={avatar} userName={userName} size={32} />
              <IconChevronDown />
            </button>
          </div>
        </div>
      ...
      {drawerOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 900 }}>
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(17,24,39,0.45)' }} onClick={() => setDrawerOpen(false)} />
          <div className="cd-drawer">
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
                <AvatarDisplay avatar={avatar} userName={userName} size={42} />
              ...
              <button onClick={() => setDrawerOpen(false)} className="cd-icon-btn" aria-label="Close">
                <IconX />
              </button>
            </div>
            ...
            <div style={{ padding: '1rem' }}>
              ...
              {quickActions.map((a) => (
                <Link key={a.title} to={a.to} className="cd-drawer-item" onClick={() => setDrawerOpen(false)}>
                  <span className="cd-drawer-icon">
                    <span style={{ color: '#2E7D32' }}>{a.icon}</span>
                  </span>
                  <span>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              ...
              <button onClick={() => setAvatarModalOpen(false)} className="cd-icon-btn" aria-label="Close">
                <IconX />
              </button>
            </div>
            ...
            >
              <AvatarDisplay avatar={avatar} userName={userName} size={56} />
              <div>
                <div style={{ fontWeight: '700', color: '#111827' }}>{userName}</div>
      ...
      <main className="cd-main">
        <div className="cd-banner">
          <div>
            ...
            <p style={{ margin: '0.5rem 0 0 0', color: '#E8F5E9', fontSize: '0.95rem' }}>
        ...
        <AIMoodInsights userToken={localStorage.getItem('token')} onBookSession={handleBookingSuccess} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div className="cd-card">
              ...
              {alreadyLogged || moodSubmitted ? (
                ...
                >
                  <div style={{ color: '#2E7D32', display: 'flex', justifyContent: 'center', marginBottom: '0.5rem' }}>
                    <IconHeart />
                  </div>
                  <p style={{ color: '#1B5E20', fontWeight: '700', margin: 0 }}>Mood logged for today!</p>
                  <p style={{ color: '#6B7280', fontSize: '0.85rem', marginTop: '0.25rem', marginBottom: 0 }}>
                      ...
                      >
                        ...
                        <span
                          style={{
                            fontSize: '0.7rem',
                            fontWeight: '600',
                            color: selectedMood?.value === m.value ? m.color : '#6B7280',
                          }}
                        >
                          {m.label}
                        </span>
              ...
              <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: '100%', height: 'auto' }}>
                ...
                {pts.map((p, i) => (
                  <circle
                    ...
                    cx={p[0]}
                    cy={p[1]}
                    r={weekData[i].mood ? 4 : 2.5}
                    fill={weekData[i].mood ? weekData[i].mood.color : '#D1D5DB'}
                    stroke="white"
                    strokeWidth="1.5"
                  />
                ))}
                ...
                <div style={{ background: '#F9FAFB', borderRadius: '12px', padding: '1rem', border: '1px solid #E5E7EB' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        <IconCalendar />{' '}
                        {new Date(upcomingSession.scheduled_time).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                        })}{' '}
                        •{' '}
                        {new Date(upcomingSession.scheduled_time).toLocaleTimeString('en-US', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </div>
                  ...
                  <button
                    onClick={() => navigate(`/chat/${upcomingSession.id}`)}
                    style={{
                      width: '100%',
                      padding: '0.75rem',
                      background: '#2E7D32',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {pastSessions.map((booking) => (
                    ...
                    >
                      ...
                      {booking.has_review || booking.reviewed ? (
                        <span
                          style={{
